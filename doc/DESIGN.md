...
NPU는 ARM Cortex-A9 HPS (또는 NIOS II 소프트 코어)와 FPGA 간의 **Avalon-MM 브릿지(LWHPS2FPGA)**를 통신망으로 사용하여 연결됩니다. 컨트롤 제어는 Avalon-MM Slave 포트를 통해 `npu_ctrl` 레지스터 맵에 접근하여 수행되고, 대용량 데이터 전송(행렬 가중치 및 입력 맵 데이터)은 **Altera MSGDMA (Message Stream DMA)** IP를 통한 **Avalon-ST (Streaming)** 방식으로 매우 고속으로 오프로드됩니다. 

소프트웨어 제어(C/C++) 측면에서는 Bare-metal 환경(Nios II) 뿐만 아니라, **Linux OS 환경(ARM HPS)** 에서도 `/dev/mem` 메모리 맵핑(`mmap`)과 캐시 우회 플래그(`O_SYNC`)를 통해 사용자 공간(User-space) 애플리케이션 포팅이 완벽히 지원됩니다.
...
## 4. Benchmark Execution Sequence & Synchronization

ARM Linux CPU (또는 Nios II) 가 NPU를 호출하고 DMA 벤치마크를 수행하는 제어 과정(`linux_software/npu_test/main.c`)은 **Status Register Polling**을 기반으로 한 엄격한 비동기 핸드쉐이킹으로 동작합니다.

1. **메모리 맵핑 (Linux)**: `/dev/mem`을 `O_SYNC` (Cache 우회 Device Memory 속성) 플래그를 주어 엽니다. FPGA `HPS2FPGA` (0x20000000, 램 영역)와 `LWHPS2FPGA` (0xFF200000, CSR 제어 영역)를 리눅스 가상 메모리 포인터에 바인딩합니다.
2. **NPU 모드 세팅**: `npu_ctrl[0] = 0x3` (Load Weight, `valid_in` 포함) 모드로 설정하고 MSGDMA Read 포인터를 통해 가중치 전송을 트리거합니다.
3. **데이터 Batch 준비**: `npu_ctrl[6] = 10` (10 배치 * 8열) 으로 전송할 전체 행렬의 배치 사이즈를 미리 세팅 (EOP 방어선 구축).
4. **실행 (Execution)**: `npu_ctrl[0] = 0x1` 구동 모드로 변경한 뒤, DMA Write(결과 수신)을 먼저 준비시키고 DMA Read(입력 송신) 포인터를 스트리밍 인가합니다.
5. **Polling 동기화 대기**: CPU Out-of-Order 실행으로 인한 타이밍 붕괴를 막기 위해, 임의의 딜레이(`usleep`)를 쓰지 않고 **MSGDMA CSR의 Busy 플래그** 와 **NPU Reg Status의 Busy 플래그** 가 모두 `0` (Idle)이 될 때까지 `while` 루프로 하드웨어를 완벽하게 대기(Polling)합니다.
