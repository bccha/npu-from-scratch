...
- **공정한 성능(Speedup) 측정:** 100 배치(Batch)의 8x8 행렬 곱셈을 `-O3` 글로벌 최적화가 켜진 800MHz ARM Cortex-A9에서 연산한 결과(`3중 for문`)와 FPGA 하드웨어 NPU에 DMA 스트리밍으로 쏴서 받아온 전체 시간을 `clock_gettime(CLOCK_MONOTONIC)`으로 엄밀히 측정하여 비교 검증.
